...
- Conservative legacy item-model override conversion for custom-model-data and damage predicates
  through Catalog, CLI and MCP.
- Texture animation metadata validation with bounded dimension and frame checks through Catalog,
  CLI and MCP.
- Migration block-state default normalization using caller-supplied generated reports through
  Catalog, CLI and MCP, filling declared defaults and rejecting unreported blocks or properties.
- Exact decoded-PNG comparison with capture-condition matching through Catalog, CLI and MCP.
- Official Java 26.3 metadata, registries, commands, vanilla inventories, datapack schema surfaces,
  resourcepack models, command trees, block states and all 161 entity metadata declarations with
  no extraction gaps. Added verified 26.3 tag resolution while retaining 26.2 and 1.21.11 support.
- Paper 26.3 build 5 (ALPHA) metadata and Javadocs package/type/member indexes. Refreshed coverage
  spans 51 Java releases and 47 Paper versions, including 39 Paper API type/member surfaces;
  missing historical surfaces remain explicit.
...
- Updated all workspace versions and internal dependencies to 0.1.9. Data version
  `2026.09.15-1` uses the common retrieval timestamp `2026-09-15T18:49:47.961Z`; downloadable
  surfaces are pinned to `v0.1.9` with regenerated byte sizes and SHA-256 hashes.
- Paper build ingestion now retains the official build channel, including ALPHA, and leaves
  unavailable channel information explicit.
...
- Java manifest ingestion now updates the catalog's latest Java alias together with the version
  index, keeping source audits and default Catalog, CLI and MCP lookups aligned.
