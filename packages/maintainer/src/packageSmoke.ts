      ...
      runCommand(
        ...
        [
          ...
          [
            ...
            "cleanCachedData();",
            'const partial = searchAll({ query: "Minecraft client ServerList swap replace save hidden servers", version: "26.2", limit: 3 });',
            'if (partial.searchComplete || !partial.unavailableSurfaces.some((entry) => entry.surface === "datapack-schema") || !partial.unavailableSurfaces.some((entry) => entry.surface === "paper-api") || partial.results.length > 3) throw new Error("fresh-cache cross-search must return explicit partial results");',
            'const partialCommands = searchAll({ query: "give", version: "26.2", domain: "datapack" });',
            'if (!partialCommands.results.some((entry) => entry.surface === "commands") || !partialCommands.unavailableSurfaces.some((entry) => entry.fetch?.kind === "datapack-schema-surface")) throw new Error("fresh-cache cross-search lost bundled commands or recovery guidance");',
            ...
            'if (hasBundledDataFile("java/block-states/26.2.json")) throw new Error("block states must not be bundled");',
            'const blockApi = await import("@minecraft-skills/catalog");',
            'if (blockApi.getBlockStateDefinition({ version: "26.2", blockId: "minecraft:oak_stairs" }).status !== "unavailable") throw new Error("uncached block state evidence must be unavailable");',
            'await fetchData({ kind: "block-state-surface", version: "26.2", fetch: localFetch });',
            'const block = blockApi.getBlockStateDefinition({ version: "26.2", blockId: "minecraft:oak_stairs", limit: 1 });',
            'if (block.status !== "available" || block.stateCount !== 80 || block.states.length !== 1 || !block.defaultState) throw new Error("bad cached block-state lookup");',
            'if (hasBundledDataFile("java/command-trees/26.2.json")) throw new Error("command trees must not be bundled");',
            'if (blockApi.getCommandDetails({ version: "26.2" }).status !== "unavailable") throw new Error("uncached command evidence must be unavailable");',
            'await fetchData({ kind: "command-tree-surface", version: "26.2", fetch: localFetch });',
            'const command = blockApi.getCommandDetails({ version: "26.2", path: ["execute", "as", "targets"], depth: 0 });',
            'if (command.status !== "available" || command.nodes[0]?.parser !== "minecraft:entity" || command.nodes[0]?.redirect?.[0] !== "execute") throw new Error("bad cached command detail lookup");',
            'if (hasBundledDataFile("java/entity-metadata/26.2.json")) throw new Error("entity metadata must not be bundled");',
            'if (blockApi.getEntityMetadata({ version: "26.2", entityId: "minecraft:armor_stand" }).status !== "unavailable") throw new Error("uncached entity metadata must be unavailable");',
            'await fetchData({ kind: "entity-metadata-surface", version: "26.2", fetch: localFetch });',
            'const entity = blockApi.getEntityMetadata({ version: "26.2", entityId: "minecraft:armor_stand" });',
            'if (entity.status !== "available" || !entity.coverage.complete || entity.fieldCount !== 22 || entity.metadata[0].serializer !== "BYTE") throw new Error("bad cached entity metadata lookup");',
            ...
            'if (getResourcepackModelSummary("java", "1.13").coverage !== "client-resourcepack-models") throw new Error("missing old resourcepack model summary");',
          ].join(" "),
        ],
        consumerDir,
        smokeEnv,
