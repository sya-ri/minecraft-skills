...
export function runPackageSmoke(options: { root: string; keepTemp?: boolean }): PackageSmokeResult {
  ...
  try {
    ...
    commands.push(
      runCommand(
        ...
        [
          ...
          "--eval",
          [
            'import { getAuthoringChecklist, getAuthoringContext, getAuthoringDiagnostic, getAuthoringGuardrail, getAuthoringPreflight, getAuthoringRecipe, getClaimPolicy, getCoverageSummary, getDataManifest, getDatapackSchemaSurface, getEvidenceBundle, getFactSurface, getIntentLookup, getOutputRequirement, getPaperApiSurface, getResponsePattern, getSupportMatrix, listAuthoringChecklists, listAuthoringDiagnostics, listAuthoringGuardrails, listAuthoringRecipes, listClaimPolicies, listFactSurfaces, listIntentLookups, listOutputRequirements, listResponsePatterns, listVersionSupport } from "@minecraft-skills/catalog";',
    ...
    commands.push(
      runCommand(
        "pnpm",
        ["exec", "minecraft-skills", "preflight", "paper-plugin", "26.2"],
        consumerDir,
      ),
    );
    ...
    commands.push(
      runCommand(
        "pnpm",
        ["exec", "minecraft-skills", "claim-policy", "paper-type-or-member-exists"],
        consumerDir,
      ),
    );
    ...
    commands.push(
      runCommand(
        "pnpm",
        ["exec", "minecraft-skills", "output-requirement", "paper-plugin-output-safety"],
        consumerDir,
      ),
    );
    ...
    commands.push(
      runCommand(
        "pnpm",
        ["exec", "minecraft-skills", "response-pattern", "paper-api-answer"],
        consumerDir,
      ),
    );
    ...
    commands.push(
      runCommand(
        "pnpm",
        ["exec", "minecraft-skills", "fact-surface", "paper-api-surface"],
        consumerDir,
      ),
    );
