      ...
      runCommand(
        ...
        [
          ...
          [
            ...
            'if (hasBundledDataFile("java/block-states/" + version + ".json")) throw new Error("block states must not be bundled");',
            'if (blockApi.getBlockStateDefinition({ version, blockId: "minecraft:oak_stairs" }).status !== "unavailable") throw new Error("uncached block state evidence must be unavailable");',
            'await fetchData({ kind: "block-state-surface", version, fetch: localFetch });',
            'const block = blockApi.getBlockStateDefinition({ version, blockId: "minecraft:oak_stairs", limit: 1 });',
            ...
            'if (hasBundledDataFile("java/command-trees/" + version + ".json")) throw new Error("command trees must not be bundled");',
            'if (blockApi.getCommandDetails({ version }).status !== "unavailable") throw new Error("uncached command evidence must be unavailable");',
            'await fetchData({ kind: "command-tree-surface", version, fetch: localFetch });',
            'const command = blockApi.getCommandDetails({ version, path: ["execute", "as", "targets"], depth: 0 });',
            ...
            'if (hasBundledDataFile("java/entity-metadata/" + version + ".json")) throw new Error("entity metadata must not be bundled");',
            'if (blockApi.getEntityMetadata({ version, entityId: "minecraft:armor_stand" }).status !== "unavailable") throw new Error("uncached entity metadata must be unavailable");',
            'await fetchData({ kind: "entity-metadata-surface", version, fetch: localFetch });',
            'const entity = blockApi.getEntityMetadata({ version, entityId: "minecraft:armor_stand" });',
