...
`getEntityMetadata({ version, entityId })` returns inherited static accessor indexes, value types,
serializers and source hashes for exactly 26.3, 26.2 or 1.21.11. Fetch `entity-metadata-surface` in
installed packages first. See [entity metadata](../../docs/ENTITY_METADATA.md) for coverage;
default values and bit semantics are not established.

`getBlockStateDefinition({ version, blockId, offset?, limit? })` returns official reported block
properties, allowed values, the explicit default state, and a bounded state page. Coverage
is 26.3, 26.2 and 1.21.11; fetch `block-state-surface` data first in packed installations. Unsupported
...
const customRecipePlan = getAuthoringPlan({
...
const fabricRenderTypes = await searchFabricApiTypes({
  gameVersion: "26.2",
  query: "LevelRenderEvents",
  limit: 10,
});
const fabricRenderMembers = await searchFabricApiMembers({
  gameVersion: "26.2",
  type: "ArmorRenderer",
  query: "register",
  kind: "method",
...
The selected aggregate POM and SHA-256-verified fat Javadoc archive provide provenance. Results
include source URLs, exact artifact/module coordinates, checksum, retrieval time, coverage, and
