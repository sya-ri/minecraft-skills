...
const minecraftLogLimitOptions: Readonly<Record<string, keyof MinecraftLogAnalysisLimits>> = {
  ...
  "--max-class-loading-failures": "maxClassLoadingFailures",
...
export async function runCli(argv: string[], output: Output = defaultOutput): Promise<number> {
  ...
  try {
    ...
    if (command === "command-details") {
      const [version, ...path] = positionalArgs(args);
      if (!version) throw new Error("command-details requires <version>");
      printJson(
        output,
        getCommandDetails({
          version,
          path,
          depth: Number(readOption(args, "--depth", "1")),
